...
const ForgotPass = () => {
    ...
    const handleSubmit = async (event) => {
        try {
            ...
            setIsLoading(false);
            if (error?.data?.message === 'EMAIL_IS_NOT_EXIST') {
    ...
    return (
        <div className="z-[9999999] bg-white">
            <div className="flex h-screen ">
                {/* Left Pane */}
                <div className="hidden lg:flex items-center justify-center flex-1 bg-white text-black">
                    <div className="w-full h-full text-center">
                        <img className="w-full h-full  object-cover" src={bg_hienmau} alt="" />
                    </div>
                </div>

                {/* Right Pane */}
                <div className="w-full bg-gray-100 lg:w-1/2 flex z-[9999999] items-center justify-center">
                    <div className=" max-w-md w-full p-6">
                        <h1 className="text-4xl font-semibold mb-6 text-black text-center">Quên Mật Khẩu</h1>
                        <h2 className="text-[16px] font-thin mb-6 text-gray-500 text-center">
                            "Hiến máu là bạn đang làm điều cao cả cho cộng đồng. Việc hiến máu tạo nên sự khác biệt giữa
                            sự sống và cái chết. Nỗ lực nhỏ của bạn có thể cho người khác cơ hội thứ hai để sống"
                        </h2>
                        {/* Register */}
                        ...
                        {count === 1 && (
                            <Link
                                to={'/forgotpasswordbyidcard'}
                                className="text-center text-sm text-blue-400 mt-1 cursor-pointer hover:underline"
                            >
                                Sử dụng phương thức khác
                            </Link>
                        )}
                        <div className="flex justify-between mt-2 text-sm text-gray-600 text-center">
